...
# # 此鼠标左键、双击、右键方法若没有找到图片，会自动跳过。
# def mouseClick_sometimes(clickTimes,lOrR,img,reTry):
#     if reTry == 1:
#             location=pyautogui.locateCenterOnScreen(img,confidence=0.9)
#             if location is not None:
#                 pyautogui.click(location.x,location.y,clicks=clickTimes,interval=0.2,duration=0.2,button=lOrR)
...
def dataCheck(sheet1):
    ...
    while i < sheet1.nrows:
        ...
        if cmdType.ctype != 2 or (cmdType.value != 1.0 and cmdType.value != 1.1 
        and cmdType.value != 2.0 and cmdType.value != 2.1
        and cmdType.value != 3.0 and cmdType.value != 3.1
